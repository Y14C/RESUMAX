...
def test_api_key(api_key: str, model_name: str) -> Tuple[bool, str]:
    ...
    """
    logger.info(f"[API TEST] Testing OpenAI API key with model: {model_name}")
    ...
    try:
        ...
        logger.info(f"[API TEST] Making test OpenAI API call to {model_name}")
        response = client.chat.completions.create(
            model=model_name,
            temperature=0.1,
        ...
        if response.choices and len(response.choices) > 0:
            ...
            if choice.message and choice.message.content:
                logger.info(f"[API TEST] OpenAI API key test successful for {model_name}")
                return True, f"API key is valid and working with {model_name}"
        
        logger.error(f"[API TEST] OpenAI API returned empty response for {model_name}")
        ...
        if "AuthenticationError" in error_type:
            logger.error(f"[API TEST] OpenAI Authentication Error for {model_name}: {str(e)}")
            return False, f"Invalid API key: {str(e)}"
        elif "RateLimitError" in error_type:
            logger.error(f"[API TEST] OpenAI Rate Limit Error for {model_name}: {str(e)}")
            return False, f"Rate limit exceeded: {str(e)}"
        elif "APIError" in error_type:
            logger.error(f"[API TEST] OpenAI API Error for {model_name}: {str(e)}")
            return False, f"API error: {str(e)}"
        else:
            logger.error(f"[API TEST] Unexpected error in OpenAI API test for {model_name}: {str(e)}")
